...
namespace Nop.Plugin.Shipping.FixedByWeightByTotal
{
    ...
    public class FixedByWeightByTotalComputationMethod : BasePlugin, IShippingRateComputationMethod
    {
        ...
        /// <param name="zip">Zip code</param>
        /// <returns>Rate</returns>
        private decimal? GetRatePerWeightUnit(decimal subTotal, decimal weight, int shippingMethodId, int storeId, int warehouseId, int countryId, int stateProvinceId, string zip)
        {
            var shippingByWeightRecord = _shippingByWeightByTotalService.FindRecords(shippingMethodId, storeId, warehouseId, countryId, stateProvinceId, zip, weight, subTotal);
            ...
            return shippingByWeightRecord.RatePerWeightUnit > decimal.Zero
                ? shippingByWeightRecord.RatePerWeightUnit
                : decimal.Zero;
        }
        ...
        public GetShippingOptionResponse GetShippingOptions(GetShippingOptionRequest getShippingOptionRequest)
        {
            ...
            if (_fixedByWeightByTotalSettings.ShippingByWeightByTotalEnabled)
            {
               ...
               // var weight = _shippingService.GetTotalWeight(getShippingOptionRequest, ignoreFreeShippedItems: true);
                var weight = _shippingService.GetHeaviestItemWeight(getShippingOptionRequest);
                foreach (var shippingMethod in _shippingService.GetAllShippingMethods(countryId))
                {
                    var rate = GetRatePerWeightUnit(subTotal, weight, shippingMethod.Id, storeId, warehouseId, countryId, stateProvinceId, zip);
