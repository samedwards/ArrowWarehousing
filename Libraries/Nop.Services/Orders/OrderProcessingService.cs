...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderProcessingService : IOrderProcessingService
    {
        ...
        protected virtual PlaceOrderContainer PreparePlaceOrderDetails(ProcessPaymentRequest processPaymentRequest)
        {
            ...
            var orderTotal = _orderTotalCalculationService.GetShoppingCartTotal(details.Cart, out decimal orderDiscountAmount, out List<DiscountForCaching> orderAppliedDiscounts, out List<AppliedGiftCard> appliedGiftCards, out int redeemedRewardPoints, out decimal redeemedRewardPointsAmount, out _);
        ...
        /// </summary>
        /// <param name="order">Order</param>
        /// <param name="sendToOwner">Send to owner email</param>
        ...
        protected virtual string GetArributeValue(string attribute, string description)
        {
            var attributeIndex = description.IndexOf(attribute, StringComparison.InvariantCultureIgnoreCase);
            var breakIndex = description.IndexOf("<br />", StringComparison.InvariantCultureIgnoreCase);
            if (attributeIndex > -1)
            {
                return attributeIndex < breakIndex 
                    ? description.Substring(attributeIndex + attribute.Length, breakIndex - (attributeIndex + attribute.Length)) 
                    : description.Substring(attributeIndex + attribute.Length);
            }
        ...
        protected virtual void SendToMailchimp(Customer customer, ExternalConfig externalConfig)
        {
            return;
            var defaultAddress = customer.BillingAddress ?? customer.ShippingAddress;
            var email = customer.Email ?? defaultAddress?.Email;
            if (email == null)
            {
                _logger.Warning("Customer has no email address, can not send to mailchimp");
                return;
            }

            var headers = new Dictionary<string, string>
            {
                { "Authorization", $"Basic {externalConfig.MailchimpApiKey}" }
            };

            var subscriberHash = CreateMD5Hash(email.ToLower());
            ...
            var json = JsonConvert.SerializeObject(
                new
                {
                    ...
                    merge_fields = new
                    {
                        FNAME = customer.GetFirstName() ?? defaultAddress?.FirstName,
                        LNAME = customer.GetLastName() ?? defaultAddress?.LastName
                    }
                });

            var customerContent = new StringContent(json, Encoding.UTF8, "application/json");

            PostCustomerMailchimp(customerContent, headers, subscriberHash);
        }

        protected void PostCustomerMailchimp(HttpContent content, Dictionary<string, string> headers, string subscriberHash)
        {
            var httpRequestMessage = new HttpRequestMessage
            {
                Method = HttpMethod.Put,
                RequestUri = new Uri($"https://us8.api.mailchimp.com/3.0/lists/1fd5c9e90e/members/{subscriberHash}"),
                Content = content
            };
            ...
            using (var client = new HttpClient())
            {
                ...
                _logger.Error($"Failed to send customer to mailchimp with message: [Response: {resultString.ToJson()}] [Request: {content.ToJson()}]");
                throw new Exception(resultString.ToJson());
            }
        }

        protected virtual void SendToDear(Order order, ExternalConfig externalConfig)
        {
            var customer = order.Customer;
            var billingAddress = customer.BillingAddress;
            var shippingAddress = customer.ShippingAddress;
            var defaultAddress = billingAddress ?? shippingAddress;
            var customerName = string.IsNullOrWhiteSpace(customer.GetFullName())
                ? $"{defaultAddress?.FirstName} {defaultAddress?.LastName}"
                : customer.GetFullName();
            var reference = $"{order.CustomOrderNumber}: ";
            ...
            if (!string.IsNullOrWhiteSpace(order.CheckoutAttributeDescription))
            {
                deliveryInstructions = GetArributeValue("Delivery Instructions: ", order.CheckoutAttributeDescription);
            }

            var headers = new Dictionary<string, string>
            {
                { "api-auth-accountid", externalConfig.DearAccountId },
                { "api-auth-applicationkey", externalConfig.DearApplicationkey }
            };

            var lines = order.OrderItems.Select(x => new
            {
                SKU = x.Product.Sku,
                Quantity = x.Quantity,
                Price = Math.Round(x.UnitPriceInclTax, 2),
            ...
            {
                var additionalFeeInclTax = Math.Round((order.OrderSubtotalInclTax + order.OrderShippingInclTax) * (order.PaymentMethodAdditionalFeeInclTax / 100), 2);
                var additionalFeeExclTax = Math.Round((order.OrderSubtotalExclTax + order.OrderShippingExclTax) * (order.PaymentMethodAdditionalFeeInclTax / 100), 2);
                lines.Add(new
                {
                    SKU = "CREDIT2%",
                    Quantity = 1,
            ...
            var json = JsonConvert.SerializeObject(
                new
                {
                    Customer = defaultAddress?.Company ?? customerName,
                    Contact = customerName,
                    Phone = defaultAddress?.PhoneNumber,
                    Email = customer.Email ?? defaultAddress?.Email,
                    ...
                    SalesRepresentative = "Arrow Warehousing Website",
            ...
            var saleContent = new StringContent(json, Encoding.UTF8, "application/json");

            if (!PostSaleToDear(saleContent, headers))
            {
                _logger.Warning("Failed to send sale to dear, creating customer");
                var customerContent = new StringContent(
                    JsonConvert.SerializeObject(
                        new
                        {
                            Name = defaultAddress?.Company ?? customerName,
                            Currency = "NZD",
                            PaymentTerm = "NOW",
                            TaxRule = "Tax Inclusive",
                            ...
                            Addresses = new[] { new {
                                ...
                                Postcode = defaultAddress?.ZipPostalCode,
                ...
                PostCustomerToDear(customerContent, headers);

                if (!PostSaleToDear(saleContent, headers))
                    throw new Exception();
            }
        }
        protected bool PostSaleToDear(HttpContent content, Dictionary<string, string> headers)
        {
            var httpRequestMessage = new HttpRequestMessage
            {
                Method = HttpMethod.Post,
                RequestUri = new Uri("https://inventory.dearsystems.com/ExternalAPI/Sale/"),
                Content = content
            };
            httpRequestMessage.Headers.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
            headers.ToList().ForEach(h => httpRequestMessage.Headers.Add(h.Key, h.Value));
            ...
            using (var client = new HttpClient()) {
                ...
                if (!resultString.Contains("400") && !resultString.Contains("404") || !resultString.Contains("ErrorCode")) return true;

                _logger.Warning($"Failed to send sale to dear with message: [Response: {resultString.ToJson()}]");
                return false;
            }
        }

        protected void PostCustomerToDear(HttpContent content, Dictionary<string, string> headers)
        {
            var httpRequestMessage = new HttpRequestMessage
            {
                Method = HttpMethod.Post,
                RequestUri = new Uri("https://inventory.dearsystems.com/ExternalAPI/Customers/"),
                Content = content
            };
            httpRequestMessage.Headers.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
            headers.ToList().ForEach(h => httpRequestMessage.Headers.Add(h.Key, h.Value));
            ...
            using (var client = new HttpClient())
            {
                ...
                if (!resultString.Contains("400") && !resultString.Contains("404") || !resultString.Contains("ErrorCode")) return;

                _logger.Error($"Failed to create customer in dear with message: [Response: {resultString.ToJson()}] [Request: {content.ToJson()}]");
                throw new Exception(resultString.ToJson());
            }
        }
        ...
        public virtual PlaceOrderResult PlaceOrder(ProcessPaymentRequest processPaymentRequest)
        {
            ...
            try
            {
                ...
                if (processPaymentResult.Success)
                {
                    ...
                    var externalConfig = EngineContext.Current.Resolve<ExternalConfig>();

                    try
                    {
                        //send order to dear
                        SendToDear(order, externalConfig);
                    }
                    catch (Exception exc)
                    {
                        _logger.Warning($"Failed to send to dear with message: [{exc.Message}]", exc);
                    }

                    try
                    {
                        //send customer to mailchimp
                        SendToMailchimp(order.Customer, externalConfig);
                    }
                    catch (Exception exc)
                    {
                        _logger.Warning($"Failed to send to mailchimp with message: [{exc.Message}]", exc);
                    }
