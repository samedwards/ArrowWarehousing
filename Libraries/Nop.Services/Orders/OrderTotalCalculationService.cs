...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderTotalCalculationService : IOrderTotalCalculationService
    {
            ...
            out SortedDictionary<decimal, decimal> taxRates, bool usePaymentMethodAdditionalFee = true)
        {
            ...
            if (usePaymentMethodAdditionalFee)// && _taxSettings.PaymentMethodAdditionalFeeIsTaxable)
            {
               ...
               var paymentMethodAdditionalFeeInclTax = _taxService.GetPaymentMethodAdditionalFee(paymentMethodAdditionalFee, true, customer, out taxRate);
                ...
                var paymentMethodAdditionalFeePercentage = _paymentService.GetAdditionalHandlingFee(cart, paymentMethodSystemName) / 100;

                paymentMethodAdditionalFeeTax = (subTotalTaxTotal + shippingTax) * paymentMethodAdditionalFeePercentage;//paymentMethodAdditionalFeeInclTax - paymentMethodAdditionalFeeExclTax;
                //ensure that tax is equal or greater than zero
                if (paymentMethodAdditionalFeeTax < decimal.Zero)
                    paymentMethodAdditionalFeeTax = decimal.Zero;
        ...
        {
            return GetShoppingCartTotal(cart, out _, out _, out _, out _, out _, out _, useRewardPoints, usePaymentMethodAdditionalFee);
        }
            ...
            bool? useRewardPoints = null, bool usePaymentMethodAdditionalFee = true)
        {
            ...
            var paymentMethodAdditionalFeePercentage = decimal.Zero;
            if (usePaymentMethodAdditionalFee && !string.IsNullOrEmpty(paymentMethodSystemName))
            {
                paymentMethodAdditionalFeePercentage = _paymentService.GetAdditionalHandlingFee(cart, paymentMethodSystemName) / 100;
                var paymentMethodAdditionalFee = 0; //_paymentService.GetAdditionalHandlingFee(cart, paymentMethodSystemName);
                paymentMethodAdditionalFeeWithoutTax =
                   _taxService.GetPaymentMethodAdditionalFee(paymentMethodAdditionalFee,
                       false, customer);
            }
            ...
            var addtionalFeeExclTax = (resultTemp - shoppingCartTax) * paymentMethodAdditionalFeePercentage;
            addtionalFee = _workContext.TaxDisplayType == TaxDisplayType.IncludingTax
                ? addtionalFeeExclTax * 1.15m
                : addtionalFeeExclTax;

            resultTemp += addtionalFeeExclTax;
