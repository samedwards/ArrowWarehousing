...
namespace Nop.Services.Shipping
{
    ...
    public partial class ShippingService : IShippingService
    {
        ...
        /// <param name="ignoreFreeShippedItems">Whether to ignore the weight of the products marked as "Free shipping"</param>
        /// <returns>Total weight</returns>
        public virtual decimal GetHeaviestItemWeight(GetShippingOptionRequest request)
        {
            if (request == null)
                throw new ArgumentNullException(nameof(request));

            return request.Items.Select(packageItem => GetShoppingCartItemWeight(packageItem.ShoppingCartItem, true)).Concat(new[] {decimal.Zero}).Max();
        }
